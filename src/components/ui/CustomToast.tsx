...
// Usage Example:
/*
import { CustomToast, useToast } from './CustomToast';

const MyComponent = () => {
  const { toast } = useGlobalToast();

  const showSuccessToast = () => {
    const toastId = toast.success('Success!', 'Your action was completed successfully', {
      position: 'top',
      duration: 3000,
  ...
  const showCustomIdToast = () => {
    // Use custom ID for easier management
    const customId = 'my-custom-toast';
    toast.loading('Loading...', 'Please wait', { id: customId });
    
    // Later, update by the same ID
    setTimeout(() => {
      toast.update(customId, {
        type: 'success',
        title: 'Updated!',
        message: 'This toast was updated by ID'
      });
    }, 2000);
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity onPress={showLoadingThenSuccess}>
        <Text>Show Loading → Success</Text>
      </TouchableOpacity>

      <TouchableOpacity onPress={showCustomIdToast}>
        <Text>Show Custom ID Toast</Text>
      </TouchableOpacity>
    </View>
  );
};
